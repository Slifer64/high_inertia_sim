...
TrainSlider::TrainSlider(double max_val, const std::string &label_, QWidget *parent): QFrame(parent)
{
  ...
  QObject::connect(slider, &QSlider::valueChanged, this, [this]()
  {
    ...
    emit sliderValueChanged(getSliderPos()); 
  });
...
void TrainSlider::launchResetSliderThread()
{
    std::thread([this]()
    {
      while (reset_slider)
...
double TrainSlider::getSliderPos()
{
  return sliderPos2RealPos(slider->sliderPosition());
}

void TrainSlider::chkBoxChanged()
{
...
void TrainSlider::updateSliderText()
{
  double r_val = getSliderPos();
  value->setText(QString::number(r_val,'f',1));
}

double TrainSlider::sliderPos2RealPos(int s_pos) const
{
...
int TrainSlider::realPos2SliderPos(double r_pos) const
{
  double s_min = slider->minimum();
  double s_max = slider->maximum();
