...
%% load train data
fid = FileIO('../training/ur_train_data_pih.bin');
...
final_pose = [Pd_data(:,end)' Qd_data(:,end)']
return

n_data = length(Timed);
norm_Qd_data = zeros(1, n_data);
for j=1:n_data, norm_Qd_data(:,j)=norm(Qd_data(:,j)); end

figure;
plot(Timed, norm_Qd_data, 'LineWidth',2);
return
